"""
Purpose:          Variance Inflation Factor (VIF) for all four model covariate
                  sets (Models A-D), computed on clinical_merged_371.csv.
Manuscript:       Methods — Covariate collinearity (sensitivity models)
Figure/Table:     Supplementary Fig. 4
Input:            CLINICAL_CSV (clinical_merged_371.csv)
Output:           results/figures/phenotyping/clinical_vif_<model>.png
...
models = {
    "Model A": ['age', 'gender', 'edu', 'bmi', 'smoking', 'alcohol', 'HTN', 'DM', 'CVD'],
    "Model B": ['age', 'gender', 'edu', 'bmi', 'smoking', 'alcohol', 'HTN', 'DM', 'CVD', 'CRP', 'egfr'],
    "Model C": ['age', 'gender', 'edu', 'bmi', 'smoking', 'alcohol', 'HTN', 'DM', 'CVD', 'Statin', 'PPI', 'Antibiotics'],
    "Model D": ['age', 'gender', 'edu', 'bmi', 'smoking', 'alcohol', 'HTN', 'DM', 'CVD', 'dm_med', 'htn_med', 'chol_med']
}
...
for model_name, features in models.items():
    ...
    if len(df_vif) > 10:
        ...
        ax.set_title(f'Variance Inflation Factor ({model_name})', fontweight='bold', fontsize=14, pad=15)
        ...
        plt.savefig(str(out_dir / f'clinical_vif_{save_name}.png'), dpi=600, bbox_inches='tight')
        plt.close()
    else:
